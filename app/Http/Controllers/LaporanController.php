...
class LaporanController extends Controller
{
    ...
    public function pilih()
    {
        ...
        if ($jenis == '1') {
            $data = Dataumum::get();
            $pdf = Pdf::loadView('superadmin.laporan.pdf_dataumum', compact('data'))->setPaper('a4', 'landscape');;
            return $pdf->stream();
        }

        if ($jenis == '2') {
            $data = Komponen::get();
            $pdf = Pdf::loadView('superadmin.laporan.pdf_komponen', compact('data'))->setPaper('a4', 'landscape');;
            return $pdf->stream();
        }

        if ($jenis == '3') {
            $data = Program::get();
            $pdf = Pdf::loadView('superadmin.laporan.pdf_program', compact('data'))->setPaper('a4', 'landscape');;
            return $pdf->stream();
        }

        if ($jenis == '4') {
            $data = Pembiayaan::get();
            $pdf = Pdf::loadView('superadmin.laporan.pdf_pembiayaan', compact('data'))->setPaper('a4', 'landscape');;
            return $pdf->stream();
        }

        if ($jenis == '5') {
            $data = Transaksi::get();
            $pdf = Pdf::loadView('superadmin.laporan.pdf_transaksi', compact('data'))->setPaper('a4', 'landscape');;
            return $pdf->stream();
        }
    }
